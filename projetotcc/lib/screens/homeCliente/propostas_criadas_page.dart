...
Future excluirProposta() async {
  ...
  var url = Uri.parse(
      'http://' + localhost + '/api_tcc/PropostaCriada/excluirproposta.php');
...
Future<List<PropostasCriadas>> downloadJSON() async {
  // Aqui precisa mudar o endereço:
  var url = Uri.parse(
      'http://' + localhost + '/api_tcc/PropostaCriada/propostascriadas.php');
  var resposta = await http.post(url, headers: {
    'Charset': 'utf-8'
  }, body: {
  ...
  if (resposta.statusCode == 200) {
    ...
    return proposta
        .map((proposta) => PropostasCriadas.fromJson(proposta))
        .toList();
  } else {
    ...
    return RefreshIndicator(
      onRefresh: () async {
        ...
        Navigator.pushReplacementNamed(
            context, '/screens/homeCliente/home_cliente_page');
      },
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Center(
                child: Text("Detalhes da Proposta:",
                    style:
            ...
            Row(children: [
              Text("Possui Seguro:",
                  style: TextStyle(fontSize: 17, fontWeight: FontWeight.bold)),
              SizedBox(width: 6),
              Text(proposta.TemSeguro == "true" ? "Sim" : "Não",
                  style: TextStyle(fontSize: 17)),
            ]),
            SizedBox(height: 9),
            Row(children: [
              Text("Precisa de Refrigeração:",
                  style: TextStyle(fontSize: 17, fontWeight: FontWeight.bold)),
              SizedBox(width: 6),
              Text(proposta.PrecisaSerRefrigerado == "true" ? "Sim" : "Não",
                  style: TextStyle(fontSize: 17)),
            ]),
            Divider(color: Colors.black, thickness: 1),
            Row(
                ...
                ElevatedButton(
                    ...
                    onPressed: () {
                      ...
                      Navigator.pushReplacement(
                        ...
                        MaterialPageRoute(
                            builder: (context) =>
                                AlterarPropostaPage(value: proposta)),
                      );
                    }),
                SizedBox(width: 5),
                ElevatedButton(
                    ...
                    onPressed: () async {
                      showDialog<String>(
                        ...
                        builder: (BuildContext context) => AlertDialog(
                          ...
                          content: Text(
                              'Você tem certeza que deseja excluir essa proposta? \n\n(essa ação é permanente!)'),
                          actions: <Widget>[
                            TextButton(
            ...
            builder: (context, snapshot) {
              if (snapshot.hasData) {
                ...
                if (proposta.toString() == "[]") {
                  return Text("Você não possui nenhuma Proposta!",
                      style:
                          TextStyle(fontSize: 20, fontWeight: FontWeight.bold));
                }
...
class _AlterarPropostaPageState extends State<AlterarPropostaPage> {
  ...
  validar() async {
    ...
    if (!_formKey.currentState!.validate()) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('Digite as informações corretamente!',
            textAlign: TextAlign.center,
            style: TextStyle(fontSize: 22, color: Colors.white)),
        backgroundColor: Color.fromARGB(255, 245, 82, 82),
        duration: Duration(milliseconds: 2000),
      ));
      return;
    } else if (dataE == "Selecione a data" || dataE == "Selecione a data") {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('Selecione as datas corretamente!',
            textAlign: TextAlign.center,
            style: TextStyle(fontSize: 22, color: Colors.white)),
        backgroundColor: Color.fromARGB(255, 245, 82, 82),
        duration: Duration(milliseconds: 2000),
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(
            'A data de Retirada deve ser menor do que a de Entrega. Confirme as datas escolhidas!',
            textAlign: TextAlign.center,
            style: TextStyle(fontSize: 22, color: Colors.white)),
        backgroundColor: Color.fromARGB(255, 245, 82, 82),
  ...
  Future alterarProposta() async {
    // Aqui precisa mudar o endereço:
    var url = Uri.parse(
        'http://' + localhost + '/api_tcc/PropostaCriada/alterarproposta.php');
    var resposta = await http.post(url, headers: {
      'Charset': 'utf-8'
    }, body: {
    ...
    if (resposta.statusCode == 200) {
      deuCerto = true;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('Sucesso ao alterar proposta!',
            textAlign: TextAlign.center,
            style: TextStyle(fontSize: 22, color: Colors.white)),
        backgroundColor: Colors.greenAccent,
        duration: Duration(milliseconds: 4000),
      ...
      deuCerto = false;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('Erro ao alterar proposta, tente novamente!',
            textAlign: TextAlign.center,
            style: TextStyle(fontSize: 22, color: Colors.white)),
        backgroundColor: Color.fromARGB(255, 245, 82, 82),
        duration: Duration(milliseconds: 4000),
        ...
        body: SingleChildScrollView(
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                SizedBox(height: 12),
                Text("Digite as informações corretamente!",
                    style: TextStyle(fontSize: 20)),
                SizedBox(height: 5),
                ...
                ValueListenableBuilder(
                    valueListenable: dropValueProduto,
                    builder: (BuildContext context, String value, _) {
                      return SizedBox(
                          ...
                          child: DropdownButtonFormField<String>(
                              ...
                              value: (value.isEmpty
                                  ? null
                                  : widget.value.TipoProduto),
                              onChanged: (escolha) =>
                                  dropValueProduto.value = escolha.toString(),
                              validator: (value) {
                ...
                ValueListenableBuilder(
                    valueListenable: dropValueCarga,
                    builder: (BuildContext context, String value, _) {
                      return SizedBox(
                          width: 375,
                          height: 93,
                          child: DropdownButtonFormField<String>(
                              ...
                              value: (value.isEmpty
                                  ? null
                                  : widget.value.TipoCarga),
                              onChanged: (escolha) =>
                                  dropValueCarga.value = escolha.toString(),
                              validator: (value) {
                ...
                Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
                  ...
                  SizedBox(width: 20),
                  ValueListenableBuilder(
                      valueListenable: dropValueUnidade,
                      builder: (BuildContext context, String value, _) {
                        return SizedBox(
                            width: 145,
                            height: 83,
                            child: DropdownButtonFormField<String>(
                                ...
                                value: (value.isEmpty
                                    ? null
                                    : widget.value.Unidade),
                                onChanged: (escolha) =>
                                    dropValueUnidade.value = escolha.toString(),
                                validator: (value) {
                ...
                Row(children: [
                  ...
                  Checkbox(
                    ...
                    onChanged: (bool? value) {
                      setState(() {
                        precisaRefrigeracao = value!;
                      });
                    },
                  ),
                  Text("Possui Refrigeração",
                      style:
                          TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                ]),
                SizedBox(height: 15),
  ...
  Future pickDateTimeE() async {
    ...
    final dateTimeE =
        DateTime(dateE.year, dateE.month, dateE.day, timeE.hour, timeE.minute);
    setState(() => this.dateTimeE = dateTimeE);
    dataE =
        '${dateTimeE.day}/${dateTimeE.month}/${dateTimeE.year}  ${timeE.hour}:${timeE.minute}';
  }
  ...
  Future pickDateTimeR() async {
    ...
    final dateTimeR =
        DateTime(dateR.year, dateR.month, dateR.day, timeR.hour, timeR.minute);
    setState(() => this.dateTimeR = dateTimeR);
    dataR =
        '${dateTimeR.day}/${dateTimeR.month}/${dateTimeR.year}  ${timeR.hour}:${timeR.minute}';
  }
